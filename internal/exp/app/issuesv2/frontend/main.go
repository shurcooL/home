...
import (
	...
	httpclientv2 "github.com/shurcooL/home/internal/exp/service/issuev2/httpclient"
	"github.com/shurcooL/issues"
	httpclientv1 "github.com/shurcooL/issuesapp/httpclient"
...
func main() {
	...
	f := &frontend{
		is:   httpclientv1.NewIssues(httpClient, "", ""),
		isV2: httpclientv2.NewIssueV2(httpClient, "", "", "/api/issuev2"),
	}
...
func setup(f *frontend) {
	...
	if !state.DisableReactions {
		...
		reactionsmenu.Setup("dmitri.shuralyov.com/scratch" /* TODO */, reactionsService, state.CurrentUser)
	}
}
...
func (f *frontend) CreateNewIssue() {
	...
	go func() {
		xxx, err := f.isV2.CreateIssue(context.Background(), issuev2.CreateIssueRequest{
			ImportPath: "dmitri.shuralyov.com/scratch", // TODO
			Title:      title,
			Body:       body,
		})
		fmt.Printf("got issue=%#v err=%v\n", xxx, err)
		if err != nil {
			// TODO: Handle failure more visibly in the UI.
			log.Println("CreateIssue:", err)
			return
		}

		// Redirect.
		// TODO
		//dom.GetWindow().Location().Href = fmt.Sprintf("%s/%d", state.BaseURI, xxx.ID)
	}()
}
