	...
	for idx, message := range chg.Messages {
		...
		Outer:
			...
			case "gerrit:setHashtag":
				added, removed, ok := parseHashtagMessage(message.Message)
				if !ok {
					log.Printf("unknown setHashtag message: %q", message.Message)
					break Outer // Try as a normal message instead.
				}
				for _, hashtag := range added {
...
// parsePSMessage parses an autogenerated:gerrit:setHashtag
// message and returns all hashtags added, removed in it.
func parseHashtagMessage(m string) (added, removed []string, ok bool) {
	for _, l := range strings.Split(m, "\n") {
		switch {
		case strings.HasPrefix(l, "Hashtag added: "):
			hashtag := l[len("Hashtag added: "):]
			added = append(added, hashtag)
		case strings.HasPrefix(l, "Hashtag removed: "):
			hashtag := l[len("Hashtag removed: "):]
			removed = append(removed, hashtag)
		default:
